...
class BacktestEngine:
    ...
    def __init__(
        ...
        self.inception_report = self._audit_inception(self.price_data)
    ...
    @staticmethod
    def _audit_inception(px, max_backfill_pct=5.0):
        """Guard against BACKFILL BIAS.

        A ticker that did not exist at the start of the window is silently
        dropped and the remaining weights renormalized, so the backtest
        reports a DIFFERENT portfolio than the one being proposed -- usually
        a flattering one. This logs a loud warning and returns the report so
        callers can restrict to the common-inception window.

        Returns dict: {ticker: {'first': Timestamp, 'missing_days': int,
        'missing_pct': float}} plus 'common_start' and 'clean' keys.
        """
        if px is None or px.empty:
            return {'clean': True, 'common_start': None, 'tickers': {}}
        start = px.index[0]
        total = max((px.index[-1] - start).days, 1)
        rep, worst, common = {}, 0.0, start
        for c in px.columns:
            ser = px[c].dropna()
            if ser.empty:
                continue
            first = ser.index[0]
            miss = max((first - start).days, 0)
            pct = 100.0 * miss / total
            rep[c] = {'first': first, 'missing_days': miss, 'missing_pct': pct}
            worst = max(worst, pct)
            common = max(common, first)
        clean = worst <= max_backfill_pct
        if not clean:
            bad = sorted(((v['missing_pct'], k, v['first']) for k, v in rep.items()
                          if v['missing_pct'] > max_backfill_pct), reverse=True)
            logger.warning(
                'BACKFILL BIAS: %d ticker(s) lack history at the window start (%s). '
                'They are dropped and weights renormalized, so early results reflect '
                'a DIFFERENT portfolio. Restrict the backtest to common_start=%s.',
                len(bad), start.date(), common.date())
            for pct, tk, fv in bad:
                logger.warning('    %-10s first=%s  missing %.1f%% of window', tk, fv.date(), pct)
        rep_out = {'clean': clean, 'common_start': common, 'tickers': rep}
        return rep_out
